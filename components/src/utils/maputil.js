...
let projector=(function () {
    var obj = {
        ...
        lngLatToPixel: function (lngLat, zoom) {
            var self = this;
            var lat = self._clip(lngLat.lat, self.datum.minExtent.lat, self.datum.maxExtent.lat);
            var lng = self._clip(lngLat.lng, self.datum.minExtent.lng, self.datum.maxExtent.lng);

            var x = (lng + 180) / 360;
            var sinLatitude = Math.sin(lat * Math.PI / 180);
            var y = 0.5 - Math.log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI);

            var mapSize = self._mapSize(zoom);

            var pixelX = parseInt(self._clip(x * mapSize + 0.5, 0, mapSize - 1));
            var pixelY = parseInt(self._clip(y * mapSize + 0.5, 0, mapSize - 1));
        ...
        pixelToLngLat: function (pixel, zoom) {
            var self = this;
            var mapSize = self._mapSize(zoom);
            var x = (self._clip(pixel.x, 0, mapSize - 1) / mapSize) - 0.5;
            var y = 0.5 - (self._clip(pixel.y, 0, mapSize - 1) / mapSize);
...
function distance(lnglat1, lnglat2) {
    var c = lnglat1.lat * Math.PI / 180.0;
    var d = lnglat2.lat * Math.PI / 180.0;
    var a = c - d;
    var b = (lnglat1.lng - lnglat2.lng) * Math.PI / 180.0;
